...
const ALLOWED_GUARDED_SUPER_REFERENCES: [(&str, &str); 7] = [
    ...
    ("src/config/instance_gitignore.rs", "root_agent"),
...
const FORBIDDEN_NAMES: [&str; 2] = ["is_root_agent_path", "ROOT_AGENT_PATH_PREDICATE"];
...
fn names_the_replaced_module(body: &str) -> bool {
    FORBIDDEN_NAMES.iter().copied().any(|forbidden_name| {
        let mut from = 0usize;
