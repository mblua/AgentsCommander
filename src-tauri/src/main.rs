    ...
    match cmd.try_get_matches() {
        Ok(matches) => {
            match agentscommander_lib::cli::Cli::from_arg_matches(&matches) {
                Ok(cli) => {
                    if let Err(error) = cli.validate_isolation_grammar() {
                        agentscommander_lib::cli::attach_parent_console();
                        if cli.isolation_status {
                            eprintln!("E_ISOLATION_ARGUMENTS");
                        } else {
                            eprintln!("Error: {error}");
                        }
                        agentscommander_lib::cli::flush_outputs();
                        std::process::exit(if cli.isolation_status { 2 } else { 1 });
                    }

                    // This is deliberately before placement resolution, logging,
                    // CLI dispatch, or GUI singleton acquisition. In isolated
                    // mode it is the only root-selection path; normal mode
                    // resolves the existing instance location once and installs
                    // that result for every later config consumer.
                    let startup_state =
                        match agentscommander_lib::config::app_state_root::resolve_startup_state(
                            cli.isolated_state_root.as_deref(),
                        ) {
                            Ok(state) => state,
                            Err(error) => {
                                agentscommander_lib::cli::attach_parent_console();
                                eprintln!("{}", error.stderr_code());
                                agentscommander_lib::cli::flush_outputs();
                                std::process::exit(2);
                            }
                        };
                    if let Err(error) =
                        agentscommander_lib::config::app_state_root::install_resolved_state_root(
                            startup_state,
                        )
                    {
                        agentscommander_lib::cli::attach_parent_console();
                    ...
                    if cli.isolation_status {
                        agentscommander_lib::cli::attach_parent_console();
                        match agentscommander_lib::config::app_state_root::isolation_status() {
                            Ok(status) => {
                                match serde_json::to_string(&status) {
                                    Ok(json) => agentscommander_lib::cli_println!("{json}"),
                                    Err(_) => {
                                        eprintln!("E_ISOLATION_PROFILE_INVALID");
                                        agentscommander_lib::cli::flush_outputs();
                                        std::process::exit(2);
                                    }
                                }
