...
description: "How the Root Agent offers Agency Agents role templates before creating any specialist agent: the mandatory offer, describing Agency Agents from real local data only (never invented), the bounded skip exceptions, the agency-templates CLI flow, and handling a missing template cache."
when_to_use: Load before creating any new specialist agent (any role-defined create-agent-matrix). Also whenever the user asks to add, create, or set up a new specialist role or agent.
---
