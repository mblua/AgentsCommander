...
11. (Round 2.) The Section 7 enumeration recipe is re-run on the final tree and every candidate it yields is classified (a), (b) or (c) per its stop-the-line criterion, with the (b) and (c) classifications recorded (Section 14.2 is the recorded baseline). The registry module doc's claim that every child of the instance config directory has a row is true under the final table. A candidate that is neither covered nor recorded fails this criterion; it does not get resolved by editing the doc comment. The four rotation rows additionally hold their narrowness: no rotation glob matches at depth (`sub/app.log.1`, `sub/activity.jsonl.1`), none matches without the literal dot (`applog.1`), and none is the rule that matches any byte-order neighbour, verified with `git check-ignore -v` rather than by inspection.

## 10. Certification

**Current status: READY_FOR_IMPLEMENTATION**, certified in recertification round 2 (2026-08-20 UTC); see the round-2 subsection at the end of this section, which is the governing one. The two subsections above it are the dated records of the earlier certifications and are superseded.
...
Recorded so the next run starts from a decided baseline rather than re-deriving one (Section 7's stop-the-line criterion, classification (b)):
