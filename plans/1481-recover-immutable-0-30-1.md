...
6. Extract only that literal member beneath the fixed install root with GNU tar using `--gzip --extract --file`, `--directory` set to the fixed install root, `--no-wildcards`, `--anchored`, `--no-same-owner`, `--same-permissions`, `--keep-old-files`, and `--no-overwrite-dir`, followed by `--` and only the literal selected member. Do not extract another member, use a wildcard, restore an owner, overwrite an existing path, or apply a post-extraction `chmod` repair.
...
- The literal SHA-256 comparison precedes the first `tar` invocation. Listing selects exactly one literal member whose regular-file mode is exactly `0755`; extraction selects only that member with no wildcard matching, owner restoration, overwrite, or chmod repair. The extracted binary is regular, non-symlink, executable, mode `0755`, link count one, and canonically equal to the fixed selected path beneath the fresh canonical install root.
