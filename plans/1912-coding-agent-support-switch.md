...
| Seed-once manifest write: any existing entry means user-owned; embedded bytes written verbatim in three arms | `ensure_seeded`, `:383-472`; raw bytes at `:426` (legacy regular file present but unreadable), `:431` (legacy file absent / a directory / a symlink), `:434` (no legacy dir, `None`) |
...
Loop body order in `validate_and_filter` (decided, the dedup-order correction from the feasibility read): validate (existing) -> `seen_keys.insert` (existing) -> support gate (new) -> push. The gate is keyed, so with the key recorded first the bookkeeping stays honest (`seen_keys` mirrors the emitted rows); either order drops every entry of a de-supported key — a second entry with the de-supported key cannot resurrect it — and no test can distinguish the two orders (round-1 note 1; see the R5 note in section 8). After-insert is kept for that honest bookkeeping, not because the gate needs it:
...
**4.4.1 Manifest bytes.** New helper, used in the THREE arms of `ensure_seeded` that today write `EMBEDDED_DEFAULT_CATALOG_JSON.as_bytes().to_vec()` — `:426` (legacy regular file present but unreadable; warn + embedded), `:431` (legacy file absent / a directory / a symlink; the standard production arm, because `ensure_seeded_for_project_with_token` passes `Some(legacy)` whenever a config dir exists), `:434` (no legacy dir, `None`):
...
| Path | With a `false` row for key K |
|---|---|
| `ensure_seeded`, embedded source (absent legacy, non-file legacy, unreadable legacy) | writes the enabled rows only; file parses; K absent from the bytes. All three arms (`:426`, `:431`, `:434`) become one-expression calls of the same `embedded_seed_bytes()` helper; the helper is probed in both directions in R8 (arm `:434`) and R15 (arm `:431`) |
...
Seed-side arm coverage (round-1 F1): R8 drives arm `:434` (no legacy dir) in the enabled control AND the `false`-row direction; R15 drives arm `:431` (legacy file absent; legacy file not a regular file) the same way. Arm `:426` (legacy regular file present but unreadable) has no portable machine probe on the Windows verification box — the verdict's chmod-000 option is unix-only and would be root-dependent on CI — and its write site is replaced by the same one-expression `embedded_seed_bytes()` call as the two probed arms; see the note under R15.
...
| R5 | `desupported_row_dropped_from_parsed_manifest_and_dedup_cannot_resurrect_it` | manifest `[muse "First", muse "Second", {key:"mine", command:"muse"}]` under `TABLE_MUSE_OFF` -> exactly `["mine"]`, `[0].command == "muse"` (control: same file with no override -> `["muse","mine"]`, label "First"). NOTE: the outcome is order-insensitive (skip before vs after the `seen_keys` insert) — R5 proves the gate on a duplicate-bearing user manifest, not the loop-order decision (see the notes under this table) |
...
| R13 | `all_seeds_present_ignores_desupported_master` | root = fresh temp project, ac = `root/.ac`; seed under `TABLE_CLAUDE_OFF` via `ensure_seeded(&ac, None)` + `ensure_seeded_masters(&ac, None)`: manifest + `.codex` + `.opencode` present, `.claude` absent. `all_seeds_present(&ac)` true inside the override, false outside it (`.claude` missing); delete `.codex` -> false inside the override too; `ensure_seeded_for_project(&root)` inside the override recreates `.codex` and still not `.claude`. Gate-wiring probe (round-1 F2): state steady again, `ensure_seeded_for_project_with_token(&root, Some(&ManifestActivationToken::for_test()))` (the `cfg(test)` constructor, `seed_manifest.rs:3205`; `ManifestActivationToken` already in module scope via the `use` at `coding_agents_catalog.rs:37-40`) inside the override returns BEFORE `acquire_project_gate_soft` — assert `root/.ac/.seed-manifest.lock` (`SEED_MANIFEST_LOCK_FILENAME`, `seed_manifest.rs:33`) was NOT created. Contrast: delete `.codex`, call again with the same token — the pre-check falls through, the lock IS created (lock files persist after `release`, `seed_manifest.rs:4667`), `.codex` is re-seeded, `.claude` still absent, manifest bytes unchanged |
...
| R15 | `desupported_row_absent_from_seeded_manifest_bytes_in_legacy_dir_arms` | under `TABLE_MUSE_OFF`, two fresh (project, legacy) tempdir pairs mirroring the shapes of the existing `legacy_absent_or_not_a_file_seeds_embedded_default`: (a) `ensure_seeded(project, Some(legacy))` with the legacy dir EMPTY — `agents.json` absent (arm `:431`); (b) same but `legacy/agents.json` is a DIRECTORY (arm `:431`, non-regular branch). Both: `ensure_seeded` returns `Some`; the file parses as `CodingAgentCatalog` with `schema_version 1` and 7 agents; the raw bytes do NOT contain `"muse"`; `load_catalog` -> 7, no muse key, `[0].key == "claude"`. Controls (same two shapes, no override): the seeded bytes are byte-identical to `EMBEDDED_DEFAULT_CATALOG_JSON` — arm `:431` keeps writing the raw resource while every row is `true`, the guarantee R8's control already gives arm `:434` |
| ext | `every_embedded_master_maps_to_a_catalog_def_with_matching_configseed` | add `assert_eq!(def.key, m.key)` |

Notes. R5 is order-insensitive by design: with the key-based drop gate, running the skip before or after the `seen_keys` insert emits the identical row set, so R5 proves the gate, not the loop order; section 4.2 keeps the after-insert order for honest per-key bookkeeping. R15 leaves arm `:426` (legacy regular file present but unreadable) machine-uncovered by decision: no portable way to make a regular file unreadable exists on the Windows verification box, and the verdict's unix-only chmod-000 probe is declined because it cannot run here and would be root-dependent on unix CI; that arm's write site is the same one-expression `embedded_seed_bytes()` call as the two arms R8/R15 probe in both directions.
...
```
Expected: 53 passed (38 baseline + 15 new), 0 failed.
...
- `coding_agents_catalog.rs` gated `== 8` / last-is-muse pins: `1233-1234`, `1247-1252`, `1312`, `1541`, `1547`, `1552`, `1685`, `1699`, `1759`, `1763`; test R2 (`builtin_agent_support_ships_every_row_enabled`).
- If K is muse: the new tests' shipped-table controls that assume muse on: R3's before/after len-8-with-muse asserts, R5's no-override control (`["muse","mine"]`, label "First"), R8's no-override control (seeded bytes == the raw resource) and its 7-no-muse asserts, R10's 8-row premise, R15's per-shape no-override controls (raw bytes on the legacy arms) and its no-`"muse"` asserts.
- If K is claude, codex or opencode: R11's no-override control (`[claude, codex, opencode]`; `.codex`/`.opencode` non-empty), R13's outside-override predicate assert (a missing `.claude` no longer makes it false once the shipped table drops claude) and the lock-probe steady-state leg, R14's seeded-under-shipped-table premise; existing pins `reseedable_commands_are_claude_codex_opencode` (1355), `ensure_seeded_masters_creates_nonempty_masters_and_preserves_edits` (1390), the reseed tests (1414-1479), `ensure_seeded_for_project_steady_state_precheck_skips_gate_and_writes` (1717).
- If K is one of the six donation keys (claude, codex, hermes, pi, opencode, antigravity): R7's no-override control (`["claude --update"]`) and the pre-existing backfill tests `load_catalog_backfills_empty_update_commands_from_embedded_default` (1864) and `load_catalog_never_overwrites_user_update_commands` (1913) whenever the shipped donation sequence comes from K.
...
## 12. Round 2 revision (2026-09-09): disposition of the round-1 verdict (CHANGES_REQUIRED)

Verdict reviewed: `20260909-232845-room12-ac-dev-rust-grinch-v4-to-room12-ac-tech-lead-v4-1912-r1-verdict-changes-required.md`, against plan commit `1e1d35f4`. Every anchor cited below was re-verified in the code at base `4ba6c99e` during this revision (git blob reads, not memory). No new design, no scope change, no application code touched; the agreed scope, the all-rows-`true` shipping state and every byte-identity guarantee are unchanged.

- **F1 (high) — seed gate proven on one of three embedded arms.** RESOLVED with the reviewer's required probe, as new test R15: it drives arm `:431` under `TABLE_MUSE_OFF` in both branches the verdict named — `legacy_catalog_dir = Some(empty)` (absent `agents.json`) and a DIRECTORY named `agents.json` (non-regular branch) — asserting the seeded bytes parse (`schemaVersion` 1, 7 agents) and do not contain `"muse"`, with a per-shape no-override control asserting byte-identity with `EMBEDDED_DEFAULT_CATALOG_JSON`. R8 already gives arm `:434` the same enabled-control + `false`-row pair. Arm `:426` (legacy regular file present but unreadable) is machine-uncovered by decision: the verdict itself labels the chmod-000 probe "Unix-only option", it cannot run on the Windows verification box, and chmod-000 is root-dependent on unix CI; that arm's write site becomes the same one-expression `embedded_seed_bytes()` call as the two probed arms (section 4.4.1, R15 note). The wrong plan anchors `425/430/432` are corrected to `426/431/434` in sections 2 and 4.4.1 (verdict note 3).
- **F2 (medium-high) — pre-check wiring not observable from R13.** RESOLVED with the verdict's probe, added as the final leg of R13: after the claude-off full seed (manifest + `.codex` + `.opencode` present, `.claude` absent), `ensure_seeded_for_project_with_token(&root, Some(&ManifestActivationToken::for_test()))` inside the override must return before `acquire_project_gate_soft` — assert `root/.ac/.seed-manifest.lock` (`SEED_MANIFEST_LOCK_FILENAME`, `seed_manifest.rs:33`) was NOT created. An ungated pre-check (still requiring `.claude`) fails that assert. Contrast control: delete `.codex`, call again — pre-check falls through, the lock IS created (lock files persist after `release`; `seed_manifest.rs:4667`), `.codex` is re-seeded, `.claude` stays absent, manifest bytes unchanged. Constructor `ManifestActivationToken::for_test()` is `cfg(test)` at `seed_manifest.rs:3205`; the type is already in module scope through the `use` at `coding_agents_catalog.rs:37-40`. R13's existing ungated-path asserts are kept.
- **Notes.** (1) Dedup placement: section 4.2 and the R5 row/note no longer imply R5 can detect a skip-before vs skip-after `seen_keys`-insert misplacement — the emitted row set is identical either way; the after-insert order is kept as honest bookkeeping. (2) Section 11 now lists the new tests' shipped-table controls: muse -> R3 before/after, R5 control, R8 control, R10 premise, R15 controls; claude/codex/opencode -> R11 control, R13's outside-override assert, R14 premise; the six donation keys -> R7 control and the pre-existing backfill tests `:1864` and `:1913`. (3) Source anchors `425/430/432` -> `426/431/434` corrected (above). (4) FE fallback: accepted as-is (manual two-copy link) per the verdict.
- **Counts and state.** New tests 14 -> 15 (only R15 was added; the F2 probe extends R13), so section 9 expects 38 + 15 = **53 passed** for `cargo test --lib config::coding_agents_catalog`. Author restores READY_FOR_IMPLEMENTATION as decision-complete; reviewer re-verdict and the user gate still precede any implementation. No blockers, no conflicting evidence, no prerequisites beyond the round-1 baselines (reproduced on the reviewer's box).
