...
- Issue: open #1577, approved specification comment #issuecomment-5444596303, and the release-owner round-two through round-seven rulings.
- Delivery path: Full.
...
- Task class: routine application/test/workflow work with elevated state-location safety impact. The accepted threat model covers accidental state relocation, a real unprivileged-Linux permission failure, short-lived Windows antivirus/file-sharing interference, ambiguous marker metadata, probe debris, startup failure visibility, and a shipped CLI reaching the known persistent configuration/log writers before that failure is surfaced. It does not include hostile-host/toolchain provenance, malicious PATH, signing/packaging provenance, destructive migration, or an untrusted build host.
...
- The interactive cached-update notice is another resolver reach, even though its named cache operation reads: `read_cached_notice()` at update_check.rs:345 calls `read_cache()`, which calls `cache_path()` at :60; `cache_path()` calls `config::config_dir()` at :56; and `config_dir()` calls the lazy `instance_location()` initializer at config/mod.rs:186. The notice also calls `load_settings_for_cli()` at update_check.rs:351 when a newer cached version exists. Under this plan that first initializer performs the real write probe, including retaining a newly created successful candidate directory.
...
- `test-reset` is doubly unreachable from a normal binary: it requires `--confirm-testeable`, and its handler rejects any current executable whose name is not the exact `TESTABLE_EXE_NAME`. Its complete `testability/reset.rs` production path has no reach to `config_dir()`, `instance_location()`, or `load_settings*`, and no config-directory filesystem write. It derives candidates from current_exe().parent(), reports through its existing JSON standard-stream path, and its only candidate mutation is remove_dir_all after validation. The apparent fs::write later in reset.rs is inside its #[cfg(test)] module. The one production `config::` edge is Windows-only by the cfg split: the `#[cfg(target_os = "windows")]` arm begins at testability/mod.rs:18-19, while the `#[cfg(not(target_os = "windows"))]` arm at :43 returns `Ok((false, None))` without that edge. At committed plan head `af75a638116d6494494535db2b2fba2786c2874b` (all line numbers in this chain are at that commit; the implementation shifts them), `testability::reset::execute_inner()` calls `crate::testability::acquire_profile_mutex_probe()` at reset.rs:91; Windows `testability::acquire_profile_mutex_probe()` calls `crate::config::profile::mutex_name()` at testability/mod.rs:25; `config::profile::mutex_name()` at profile.rs:83 calls `binary_suffix()` at :85; and `config::profile::binary_suffix()` at profile.rs:20-30 resolves `current_exe()` through its own separate suffix `OnceLock`. That edge terminates in the separate suffix cache and does not initialize the location resolver. Dispatch is direct at cli/mod.rs:440, `Commands::TestReset(args) => crate::testability::reset::execute(args)`. The only relevant resolver reaches in cli/mod.rs are `load_settings()` at :380 and `config_dir()` at :386, both inside validate_cli_token, which tokenless `TestReset` never calls. After the guarded block, `handle_cli` only flushes stdout/stderr before returning. Both independent round-seven complete call-trace reviews confirmed that there is no fourth resolver-reaching path.
...
- One public opaque config-startup preflight in lib.rs, called by every CLI command except exactly `test-reset` before logging and by lib::run before GUI logging/state, plus a typed lib::run result and fatal presentation in main. One `matches!(&cmd, Commands::TestReset(_))` predicate and one enclosing block guard all three resolver-reaching prologue operations together: the preflight, logger initialization, and the interactive cached update-notice block. Exactly `test-reset` skips that entire block so it can repair the corrupt candidate without first initializing the resolver.
...
- Any edit to src-tauri/tests/cli_test_reset.rs. Its existing file-target test remains unchanged evidence for the piped-stderr handler path, but it does not exercise the interactive cached-notice path. No PTY-backed test or eighth implementation file is required.
...
- In the `Some(cmd)` branch, retain console attachment and the existing in-process `AC_MACHINE_OUTPUT` classification. Use exactly one outer `if !matches!(&cmd, Commands::TestReset(_)) { ... }` predicate/block as the sole exception boundary. Place all three of these existing-order operations visibly inside that one block: (1) `agentscommander_lib::preflight_config_startup()` with its current fatal-error exit, (2) `logging::init_logger()`, and (3) the complete existing interactive cached update-notice block, including its terminal predicate and `read_cached_notice()` call. Do not give the three operations separate `TestReset` guards. After the shared block, dispatch every command through the existing `handle_cli(cmd)` path. Thus every command other than `TestReset` preflights before logger/notice/handler; exactly `TestReset` reaches only the handler after console and machine-output classification.
...
This is a closed exception for one command, `test-reset`, not a principle for recovery commands or commands that delete without writing config state. Skipping only the preflight is forbidden because `init_logger()` creates `.gitignore` and `app.log`. Skipping preflight and logger but leaving the interactive notice is also forbidden: `read_cached_notice() -> read_cache() -> cache_path() -> config_dir() -> instance_location()` can become the first initializer, and this plan puts the real create/write/delete probe inside that initializer. With both candidates initially missing, that path would create and retain `.agentscommander_testeable`; the reset handler would then report the directory it just created as `deleted` rather than `missing`. The notice’s later `load_settings_for_cli()` is a second config reach. Therefore the preflight, logger, and complete cached-notice block must stay visibly together behind the single predicate; a future refactor must not be able to relocate one bypass without the other two remaining beside it.

The three-bypass boundary is sufficient rather than merely deeper. After it, `handle_cli` dispatches `TestReset` directly to `testability::reset::execute`. The complete reset module has no reach to `config_dir()`, `instance_location()`, or `load_settings*`, and no config-directory filesystem write. It derives both candidates from the executable parent. Its Windows-only `config::profile::mutex_name() -> binary_suffix()` edge terminates in the separate suffix `OnceLock`, not the location resolver, as the committed-head chain above records. Its CLI variant has no token, so it never enters validate_cli_token, which owns cli/mod.rs’s only `load_settings()` and `config_dir()` calls. Nothing downstream of the guarded prologue can initialize the resolver on this path. Console attachment and `AC_MACHINE_OUTPUT` classification remain outside the block because they do not reach config state.
...
5. src-tauri/src/main.rs
   - CLI preflight before logger/notice/handler for every command except exactly `test-reset`; one predicate and one block make that one recovery command skip preflight, logger, and the complete cached update-notice block together. GUI Result handling still uses existing fatal presentation and exit 1.
...
7. Add the sole public lib.rs config-startup preflight; call it before CLI logging/notice/dispatch for every command except exactly `test-reset`. Put preflight, logger initialization, and the complete cached update-notice block behind one `matches!(&cmd, Commands::TestReset(_))` predicate in one block, with no duplicate guards. Call the preflight as lib::run’s first operation. Make run fallible, extract private app-outbox preparation, and keep presentation/termination in main.
...
- Require source review to prove the one `TestReset` predicate encloses all three bypasses together: `preflight_config_startup()`, `logging::init_logger()`, and the complete cached update-notice block. Three separate guards, two guards plus an unguarded notice, or a notice call relocated outside the shared block fails acceptance.
...
The accepted residual gap is explicit: `cli_test_reset::file_target_refuses_and_deletes_nothing` passes unchanged but proves nothing about the interactive cached-notice path. Its helper at tests/cli_test_reset.rs:49-56 uses `Command::output()`, so stderr is piped, `is_terminal()` is false, and the notice block is never entered. The unchanged test proves the existing piped-stderr recovery result only. The single-predicate/single-block source shape is the durable guard for the interactive path and is not covered by an automated assertion. A PTY-backed negative test is explicitly not required because the release owner ruled it disproportionate.
...
3. After fixture setup, snapshot `bin` and `cli-root` recursively by relative name, entry kind/reparse target, regular-file length, and SHA-256. Exclude timestamps and keep stdout/stderr capture logs outside those observed roots. This snapshot is the zero-net-delta baseline.
...
7. Recompute the two-root snapshot and require byte-for-byte equality with the baseline. Also name the critical negative assertions: no adjacent candidate, app.log, .gitignore, probe file, token, daemon, instance, outbox, settings, or other entry appeared. The pre-existing binary and broken marker are unchanged. This asserts zero net filesystem delta across the two observed roots; fixture setup/capture/cleanup writes are outside the measured interval or observed roots. It does not claim to observe a transient entry created and removed, or identical bytes rewritten and restored, within the invocation.
...
The zero-net-delta assertion is sufficient for the ordering defect this case guards at the pinned tree. The CLI branch has no filesystem writer before `logging::init_logger()`. A preflight misplaced after that call necessarily reaches both current writers: `ensure_instance_gitignore()` creates the selected config directory and directly creates/writes or appends `.gitignore`, and `init_logger_inner()` creates the directory, opens `app.log` with create-plus-append, and retains the handle. Production `instance_gitignore.rs` has no temp-file, rename, remove, or cleanup path; its sole `remove_file` is test-fixture code. This fixture starts without the adjacent candidate or `app.log`, so the opened log length is zero and logger rotation cannot run. The misplaced-preflight defect therefore leaves `.gitignore` and `app.log` behind, and the final snapshot detects it directly even if some unrelated transient artifact were also produced.

The limit is explicit: a future writer that created and removed an entry, or restored an existing entry to identical snapshotted bytes, within one invocation would not be caught. No such path exists on this CLI path at or before `init_logger()` today. Introducing one requires the acceptance mechanism to return to architecture review. Do not add a live filesystem watcher in this required check: it cannot attribute an event to the child, so an ambient Defender, Windows Search, or other-agent event under either root is indistinguishable from a product mutation and can block every repository merge. Defender real-time protection is off on the development box, so a local green run cannot establish that watcher's CI false-positive rate. Because the extant defect is directly observed through guaranteed persistent artifacts, that unvalidated merge-blocking risk is disproportionate and out of scope; the residual snapshots remain the guard.

The junction premise is measured, not assumed. Two independent Rust probes compiled with `rustc 1.97.1` and run on Windows NTFS produced the same results for the broken directory junction at `portable.txt`: `read_link` returned `Ok` and preserved the missing target; `symlink_metadata` returned `Ok` with `is_symlink = true`, `is_dir = false`, `is_file = false`, and attributes `0x00000410`; the follow-up `metadata` returned `Err` with `ErrorKind::NotFound`, raw OS 2, and the OS reason “The system cannot find the file specified.” Entry metadata therefore succeeds: this is not the entry-level `NotFound` that means `Absent`. Target resolution fails, so the production contract classifies the marker `Indeterminate` and supplies `WriteProbeOutcome::NotRun`, rather than taking an unsupported-entry shortcut or running the write probe.
...
- main’s binary-to-library calls to run and cli already exist; Cargo crate dependencies cannot be cyclic. The exact `Commands::TestReset(_)` guard only groups existing preflight, logger, and update-notice calls and adds no module pair. No new crate dependency is added.
