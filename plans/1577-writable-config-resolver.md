...
- `test-reset` is doubly unreachable from a normal binary: it requires `--confirm-testeable`, and its handler rejects any current executable whose name is not the exact `TESTABLE_EXE_NAME`. Its complete `testability/reset.rs` production path has zero `config_dir`, `config::`, `settings`, or `load_settings` reaches; it derives candidates from current_exe().parent(), reports through its existing JSON standard-stream path, and its only candidate mutation is remove_dir_all after validation. The apparent fs::write later in reset.rs is inside its #[cfg(test)] module. Dispatch is direct at cli/mod.rs:440, `Commands::TestReset(args) => crate::testability::reset::execute(args)`. The only config reaches in cli/mod.rs are `load_settings()` at :380 and `config_dir()` at :386, both inside validate_cli_token, which `TestReset` never calls because it takes no token.
