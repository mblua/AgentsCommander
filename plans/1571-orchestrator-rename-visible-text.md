...
- **(E) Documentation.** Documentation prose says Orchestrator; documentation that names an on-disk identifier keeps naming the current on-disk value; and one line of prose that names no on-disk identifier, `docs/reference/architecture.md:731`, keeps its bare `coordinator` on Rule P0 clause R2's phasing ground (D13, R12), because Phase 2 renames the type it describes to `SelectionArbiter` and not to Orchestrator.
...
**The doc-comment carve-out, stated so Phase 2 does not reopen the hole.** A `///` comment is out of scope when the only things that read it are a human reading the source and `rustdoc`. It is **in** scope when `clap` derive compiles it into user-visible help text, which happens in four places: on a `#[derive(Subcommand)]` variant, where it becomes that subcommand's `about`; on a `#[derive(Parser)]` or `#[derive(Args)]` struct, where it becomes that command's `about`; on a field of such a struct, where it becomes that argument's `help`; and on a `#[derive(ValueEnum)]` variant, where it becomes that possible value's help. The discriminator is not the comment syntax but whether `clap` prints it, and it is the same discriminator §3.3 already applies to `after_help` and `long_about`. Only the five `cli/mod.rs` sites listed in (a) meet it today, and the sweep that establishes that is stated there. The fourth shape has no live instance and does not move the bound: the tree's three `ValueEnum` derives are `cli/loop_cmd.rs:40` (`BusyCoordinatorCli`), `cli/coding_agent.rs:86` (`CliBackendKind`) and `cli/terminal_snapshot.rs:58` (`SnapshotFormatArg`), and no variant of any of the three carries a doc comment at all, so (a)'s bound of five stands unchanged. It is named because this carve-out binds Phase 2 and because `loop_cmd.rs:40` is the enum behind `--busy-coordinator` (`:82`, `:104`), which is where a later phase adding help text would create the first instance. A reviewer checking (e) should read it as "not printed", never as "not a doc comment".
...
After the change, `git grep -n -i coordinator -- "*.md" ":(exclude)plans/" ":(exclude)CHANGELOG.md" ":(exclude)src-tauri/" ":(exclude)src/"` must return **exactly these 59 lines and no others**. 58 of them keep `coordinator` only inside the identifier named in the row. The 59th, `docs/reference/architecture.md:731`, keeps a bare `coordinator` preserved on Rule P0 clause R2's phasing ground (D13, R12), and for that line membership in this list is the whole criterion.
...
**AC2, the documentation preserve set is exact.** `git grep -n -i coordinator -- "*.md" ":(exclude)plans/" ":(exclude)CHANGELOG.md" ":(exclude)src-tauri/" ":(exclude)src/"` returns exactly the 59 lines listed in §5.6.2, no more and no fewer. 58 of those lines carry the identifier §5.6.2 names for them. The 59th is `docs/reference/architecture.md:731`, whose §5.6.2 cell names no identifier for the surviving match, because nothing resolves the bare `coordinator` there and the line is preserved on Rule P0 clause R2's phasing ground (D13, R12); for that one line the criterion is membership alone, exactly as residual R1 states it.
...
2. **Classify** each enumerated occurrence by **Rule P0** (§5.2). **The operative test is referential, not structural.** Ask whether **something resolves the token**: code binds or reads it, a file or directory with that literal name exists on disk, a CLI parser accepts that flag, or a manifest, event, settings or i18n consumer keys on it. **PRESERVE** when something does. **RENAME** when nothing does and the token only illustrates a shape to a reader, **including when the illustration is shaped like a path, a filename, a directory, an agent name or a key**.
   ...
   **Clause R2, a referent a later phase brings into being.** A token whose referent a later phase creates or renames in step with this text is PRESERVE though nothing resolves it today. Two occurrences rest on it, `docs/features/terminal-snapshots.md:168` and `docs/reference/architecture.md:731` (§5.6.2, D13). Neither carries any other T1 occurrence, so neither line ever enters the diff and this step never pairs either; the clause is stated so a reviewer applying Rule P0 off-diff does not conclude §5.6.2 is the defect on either of them. Round 5 read this clause as covering exactly one occurrence and round 7's D13 supersedes that reading; the clause's shape is unchanged, since it never keyed on cardinality.
