...
- Round 3. Score 39, Lite band. `PARTITION: 1 phase`. Class: design-bearing. Owner: dev-rust.
...
2. If it is `Some(d)`, `Instant::now() >= d` and `state.active_producers == 0`: set `state.terminating = true`, `notify_all` on `work_available` and `state_changed`, and exit through the existing decrement path.
...
The drain loop (`:1066-1088`) and the snapshot (`:1090-1162`) stay unchanged.

### 3.3 Worker count

`worker_count` counts live logical workers. It is never overwritten:

- `ensure_workers` (`:965-1038`): keep the reap of finished handles (`:970-984`). Change the spawn loop condition (`:989`) from `workers.len() < CAPACITY` to `state.worker_count < CAPACITY`, read under the state lock. Before each `spawn`, under the state lock, `worker_count += 1`. On `spawn` `Err`, `-= 1` (underflow guard) and break. The injected `fail_worker_spawn` break (`:990-1007`) stays before the increment.
...
- `session/selection.rs` (including `spawn_blocking` at `:2848-2858`) and the test wait/assert mismatch (#1582): out of scope.
...
- **T5 Drop helper.** Registry with 4 workers, one running an item blocked on a release channel. Under the state lock set `terminating = true` and `notify_all`, as `Drop` does. `join_idle_workers_until(now + 1 s)` returns exactly 1 handle. Release the item and join that handle. Contract test, not a pre-fix detector.
...
- V3 differential proof, `cargo test --locked --lib pty::container_backend::tests::<name>`, record output:
  ...
  (c) Full change: T1-T6 green.
  (d) (c) with rule 2 ignoring `active_producers`: T1b red. (c) with step 1 using only the control deadline: T4 red at the 200 ms sample.
  Restore after each control. Commit tests and fix together. Paste the red and green excerpts into the PR body.
- V4 census: `git grep -n "request_shutdown(" src-tauri/src` and confirm §3.2's list. `git grep -n "worker_count" src-tauri/src/pty/container_backend.rs` shows no assignment other than `+= 1` / `-= 1`. `git grep -n "\.wait(" src-tauri/src/pty/container_backend.rs` shows none inside `container_shutdown_worker`.
