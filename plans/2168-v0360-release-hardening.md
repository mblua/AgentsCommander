# Release v0.36.0 implementation record

Issue: #2168. The user requested a routine release from current origin/main,
with explicit authorization to publish the GitHub Release and the npm version,
and to push the annotated tag once required CI is green.

The unchanged existing publication workflow consumes the exact eight files in
`docs/releases/v0.36.0/`. The public V1 candidate generator collected live Git,
GitHub and npm evidence twice before the version bump; both passes agreed.
The bundle covers the 16 merged scope PRs recorded in its input manifest.
Preparation base: `90be1b83` (merge of #2169, which filled Unreleased).
...
SHA256SUMS digest: `28f0bc40786a1f579d9930bbcc2a67a9b744f4547ae5b32c61985494dcd7e1a1`.

Version choice: minor. The scope adds a user-facing feature, the update toast's
Copy button (#2135), alongside fixes, docs and maintenance.
...
for the `0.36.0` version-anchored section.

Validation: version:check, evidence SHA-256 and exact Git-byte verification,
and all required PR CI before landing. The documented administrator self-review
exception applies because no second human reviewer is configured; it does not
waive any failed check. No independent review receipt or GUI acceptance result
is claimed.
