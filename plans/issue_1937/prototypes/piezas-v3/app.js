/* ============================================================================
   Prototipo v3 de candados — lógica de la maqueta (derivada de v2, sin tocar v2).
   V3 (delta visual pedido por el usuario sobre v2): la lista informativa por tipo
   muestra ÚNICAMENTE las filas protegidas, para ocupar menos líneas. El recuento
   N of M protected, los destinos de apply/+ lock/remove lock, el cartel de
...
const VIEWS = [
  ...
  },
  {
    ...
    sub: "Alcance masivo con + lock: la lista informativa muestra solo las réplicas protegidas del tipo, cada una con su par; las no protegidas quedan contadas y siguen elegibles para aplicar.",
    setup() {
      state.ctx.open = false;
      openModal("r12-ui");
    ...
    notes: [
      ...
      ["Preview de conflictos", "La lista informativa muestra solo las filas <em>Protected</em>. Con Apply aparece el cartel de conflictos si hay bloqueadas."],
  ...
  {
    id: "resultado-unlock-room",
    label: "10 · Resultado: room desbloqueada",
    title: "Entire room — resultado de quitar los candados",
    sub: "Recorrido real: en la vista 9 se elige Remove lock from → Entire room y se ejecuta. Esta vista es el estado que deja esa acción: éxito y cantidad, 0 of 4 protected, acción inactiva, pares conservados y lo de afuera sin cambios.",
    setup() {
      state.ctx.open = false;
      openModal("r12-ui");
      /* Antes: la room tiene 3 de sus 4 réplicas protegidas; afuera (room-15) conserva la suya. */
      replicaById("r12-ui").locked = true;
      replicaById("r12-rust").locked = true;
      replicaById("r15-ui").locked = true;
      state.modal.scope = "replica";
      state.modal.withLock = false;
      state.modal.applied = null;
      state.modal.futureOpen = true;
      state.modal.removeScope = "workgroup";
      /* Acción real de la maqueta, la misma que dispara el botón de la barra. */
      removeLocks();
    },
    notes: [
      ["El resultado de la acción real", "Esta vista no inventa un cartel nuevo: su setup ejecuta la <strong>acción real de la maqueta</strong> (<em>Remove lock from → Entire room</em>) y muestra el estado posterior. El <em>antes</em> es la vista 9."],
      ["Éxito y cantidad desbloqueada", "El aviso verde de la barra resume el resultado: <strong>Lock removed from 3 replicas · Coding Agent + Profile kept · no restart</strong>; el toast lo repite al instante: <em>3 locks removed — pairs kept, no restart.</em>"],
      ["Recuento nuevo y estado Unlocked", "El chip del alcance queda en <strong>0 of 4 protected</strong> y las tres opciones conservan su recuento (<em>This replica 0 protected · All replicas of this kind 1 of 2 protected · Entire room 0 of 4 protected</em>). Las 4 réplicas de la room pierden el chip <em>KEEP</em>: quedan <strong>Unlocked</strong>."],
      ["La acción queda inactiva", "Sin protegidas en el alcance, el botón pasa a <strong>Nothing to remove</strong> y queda deshabilitado; la nota avisa <em>No protected replicas in this scope — nothing to remove</em>."],
      ["Pares conservados, afuera y default intactos", "Quitar candado no reescribe: la enfocada conserva su par <em>Codex · Profile A</em> y no hay reinicio. La réplica de <strong>room-15-dev-team</strong> mantiene su candado (por eso el alcance de tipo sigue en <em>1 of 2 protected</em>) y el default de futuras réplicas sigue en <strong>Start locked</strong>."],
    ],
    annots: [
      { find: ".selection-lock-remove-scopes", pad: 4, pin: "bl", n: 1 },
      { find: ".selection-lock-remove-done", pad: 4, pin: "bl", n: 2 },
      { find: ".selection-lock-state", pad: 4, pin: "tl", n: 3 },
      { find: "#lockRemoveBtn", pad: 3, pin: "tr", n: 4 },
      { find: ".selection-lock-pair", pad: 3, pin: "tl", n: 5 },
      { find: ".selection-lock-future", pad: 4, pin: "bl", n: 5 },
    ],
  },
];
...
function renderLockBar() {
  ...
  if (state.modal.scope === "kind") {
    /* V3 (delta visual): esta lista es informativa, así que solo pinta las filas
       protegidas. La filtración es de render: kindTargets (y con él los recuentos,
       apply/+ lock, remove lock y el cartel de conflictos) sigue completo. */
