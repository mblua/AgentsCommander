/**
 * Captura las vistas del prototipo y audita la geometría de la maqueta.
 *
 * Usa Chrome headless por CDP (sin instalar nada): abre cada `index.html?vista=<id>`,
 * mide rectángulos reales (ventana, modal, barra del candado, botonera, listas),
 * detecta desbordes/textos cortados y escribe `vistas/<id>.png` (escala 1.5).
 *
 * Uso:  node capturar.mjs [--solo-auditar] [--vista=<id>[,<id>...]]
 *       (sin --vista captura las 6; con --vista=futuro solo la afectada)
 */
...
const VIEWS = ["entrada", "replica-abierto", "replica-cerrado", "tipo-preview", "resultado", "futuro"];
...
async function main() {
  ...
  for (const view of views) {
    const url = `file:///${join(here, "index.html").replace(/\\/g, "/")}?vista=${view}`;
