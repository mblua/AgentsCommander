...
Certified by `architect` at Step 7 round 3, as corrected on the locality clause and consistency-swept. Four digests are **superseded**: round 1 `FD9E52B772DAAC360DA773614F957596F21BED0C44778A7B1E18422D12577AD4`, round 2 `C76B945165A30A32315177C7E4D55D7C4E063FC19ED6449D4BBCE6445A5DCA18`, round 3 `1C5FD18BB0BF9BFCE74F95B6BB1150E1324B89BD8922BBA82F2821BD18AEB1C2`, locality correction `08ED488A6B9C820EF5AD452BB4F6DC0D0C6567674B2FEBE23BDB2663ED7D6975`.

**The code is done and correct.** Batches 1 through 4 landed and passed review; every defect found since has been in documentation wording that this plan specifies, not in the implementation. `docs/security.md:14` and `PRIVACY.md:29-31` are rewritten to be bounded and non-categorical (§4.6, §4.2, standard in §4.6.1), with locality stated as a default rather than unconditionally (§13.6), gates rebuilt around both (§9.3 rule 9), and both shipping as **batch 5** (§8.7). Both replacement texts are validated on truth and have been byte-stable since the locality correction. Records: §13 for round 1's enrichment resolution (`dev-rust` E1-E7, `dev-rust-grinch` G1-G7), §13.4 for round 2, §13.5 for round 3, §13.6 for the locality correction, §13.7 for the plan-internal consistency sweep.
...
**Superseded round-3 text. Never shipped, rejected on the locality clause. Do not ship it either:**
...
| "**local by default**" replacing "**entirely local**" | The default destinations are all local: `phone/messaging.rs:160-164` and `:166-170` for `messaging/`, `cli/send.rs:827`/`:1091` for the replica queue, `lib.rs:996-998` for the instance queue, `api/message_store.rs:722-724` for the API database. **"by default" is the locality correction.** `entirely` was a categorical, and `cli/send.rs:1078-1079` accepts a `--outbox` path with no validation, so a UNC or mounted remote destination makes "entirely local" false under a documented invocation. | AC choosing a non-local destination on its own. Nothing does. **Not** falsified by a user-selected remote destination, which the third sentence now states outright. |
| "AC sends message content to no service of its own" replacing "No external network calls are made" | The narrower claim the coordinator identified as the true residue of the old guarantee, and the part worth keeping in a privacy policy. Message delivery is `phone/mailbox.rs`'s PTY injection and filesystem writes plus the API's local DB queue; **no messaging path posts message content to any endpoint AC operates**. `PRIVACY.md`'s own "Network Features" section separately scopes Telegram and Gemini, which are opt-in, user-initiated, and not the inter-agent messaging system. | AC transmitting message content to a service it runs or contracts. Nothing does. The phrase "of its own" is deliberate: it survives a user attaching the Telegram bridge to a session, which is already documented one section above. |
| "A destination you select yourself, such as `send --outbox`, can place a message outside those locations, including on another machine" | `docs/reference/cli.md:128` documents `--outbox` as a public ordinary-message option; `cli/send.rs:1078-1079` takes it as `PathBuf::from(outbox_path)` with **no validation of any kind**, and `:1093-1098` creates the directory and writes `<msg_id>.json` into it. A UNC or mounted network path therefore puts message JSON on another host. **"such as" is exemplifying, not exhaustive**, so a second caller-selected destination would not falsify it. | Nothing plausible. Adding path validation would make the sentence over-cautious rather than false, and it would then be corrected by the #1195 audit rather than by a falsification. |
...
| "other delivery paths keep message content in queues of their own" | Existential, and it enumerates nothing. True of the API DB queue (`api/README.md:6` calls it "the durable DB queue"; `message_store.rs:785-792` stores the body) and of the caller-supplied outbox (`cli/send.rs:1077-1079`, `:1093-1098`). **This clause names neither**; cataloguing the full set is #1195's job. It only has to be true, and it stays true if their number changes. Note the third sentence of the same paragraph does name `send --outbox`, as an **example** of a caller-selected destination rather than as an entry in a list of paths; the two are compatible because neither claims completeness. | Nothing plausible. If every other path were removed the clause would be vacuous, not false. |
| "on your own disk" dropped from the middle sentence | Round 2 carried it there. Removed because `send --outbox` writes wherever the caller points, so a mid-sentence universal about the destination is one more thing a future feature can falsify. Round 3 argued the locality guarantee was still carried by "entirely local" and "No external network calls are made"; **that reasoning was wrong**, because those two clauses were themselves unconditional. The correction replaces them rather than leaning on them. | n/a. This is a removal. |
| No `~/.agentscommander/` path | **Deliberate.** The two `messaging/` roots do not share a parent: a workgroup's is in the project workspace (`phone/messaging.rs:160-164`, `mailbox.rs:717-719`), the Root Agent's is under the config directory (`config/root_agent.rs:626-628`, `:722`). Naming either root here would make the sentence false for the other half; naming both is threat-model detail. This line stays shaped by **artifact**, and `docs/security.md:14` (§4.6) carries the **roots**. `PRIVACY.md:5` makes a separate claim about configuration and session data; **leave that line alone because it is outside this change's four lines, not because it is correct.** It hard-codes `~/.agentscommander/`, while `config/mod.rs:108-112` resolves the config directory next to the executable as `.<binary_file_stem>` for a portable instance and reaches `$HOME` only as the fallback at `:137-142`. That defect is routed to **#1195** (§13.7); this plan neither fixes it nor endorses it. | n/a. |

The replacement is 3 lines for 3 lines, so `PRIVACY.md` stays at 54 lines.

**The round-3 dissent is now closed, and it was closed against me in one direction.** Round 3 preserved "entirely local" and "No external network calls are made" verbatim on the coordinator's instruction, and I recorded a dissent noting that a user can weaken both without AC changing. I then reasoned that neither was *false as written* because neither is AC calling an external service. **That reasoning does not survive.** `PRIVACY.md`'s subject is where user data goes, not which function AC calls: `send --outbox \\host\share\...` is a documented invocation under which message JSON physically leaves the machine, so an unconditional "entirely local" is false, full stop. Filing it as dissent rather than as a defect was the wrong call. The coordinator has since retracted the instruction to preserve the clauses and prescribed the qualification applied above.

**What survives from that dissent, and is now in the text.** The second half was about `apiServerBind` widening off loopback (`api/README.md:14-16`), which lets message bodies cross a network **inbound** to reach AC. The replacement handles it without naming it: "AC sends message content to no service of its own" is a claim about **outbound** transmission by AC, which an inbound API bind does not touch, and "local by default" is true of a loopback default that the operator can widen. Neither clause is falsified by that configuration, and the deployment-level question belongs to **#1195**.
...
**Superseded round-3 text. Never shipped, rejected on its opening locality clause. Do not ship it either:**
...
**The round-3 standard, set by the user and relayed by the coordinator: bounded and non-categorical.** Describe the default AC-managed locations without asserting they are the only ones. The acceptance test applied to every clause below is: **would this sentence still be true if a sixth store existed that nobody has audited?** If no, it was rewritten. That is why the `docs/security.md` replacement uses "you will find" rather than "everything is", states outright that neither list is exhaustive, and names overrides and optional features as a **category**, naming neither the API database nor `--outbox`. Cataloguing the stores themselves belongs to #1195. (`PRIVACY.md` does name `send --outbox`, added by the locality correction as an **example** of a caller-selected destination. That is an illustration of an exception, not an entry in an enumeration of stores, so it does not re-enter the trap; see §11 decision 4d.)
...
| Clause | Backing fact | What would falsify it |
| --- | --- | --- |
| "AC keeps its state in files, on this machine by default" | Every artifact named below, plus every store found during the audit rounds including the API message database, is a file that AC resolves under a local root: `config/mod.rs:169-176` for the config root, `workspace_dir` / agent-root arguments for the workspace side. **"by default" is load-bearing and was added by the locality correction.** `cli/send.rs:1078-1079` takes `--outbox` as `PathBuf::from(outbox_path)` with no validation of any kind, and `:1093-1098` creates that directory and writes the message JSON into it, so a caller-supplied UNC or mounted network path puts the file on another machine. `docs/reference/cli.md:128` documents the option publicly, which makes it product behaviour rather than misuse. | A store AC places off this machine **without being told to**. None exists. It is **not** falsified by `--outbox` pointing at a network share, which is what the unconditional round-3 phrasing "AC keeps its state in files on this machine" got wrong. **Note it also does not say "only" files, or "plain" files**: the API database is a file too, and this clause survives it. |
...
| "optional features and per-call overrides can put it outside either root **and need not keep it on this machine**" | Verified concretely, though the mechanisms are deliberately not named **in this line** (`PRIVACY.md` names `--outbox` as an example; see §11 decision 4d): `send --outbox` is public (`docs/reference/cli.md:128`), accepts any path with no validation (`cli/send.rs:1078-1079`), and creates and writes there (`:1093-1098`), so the destination can be a UNC or mounted remote path; and the opt-in control-plane API stores message bodies in `<config-dir>/api-message-bus.sqlite3` (`api/message_store.rs:15`, `:722-724`, `:785-792`) and dispatches from that queue without creating a `messaging/` file (`api/dispatcher.rs:180-190`, `:331-337`). The clause is a **category**, so it stays true if either mechanism changes and it is already true of any third one. **The "need not keep it on this machine" half is the locality correction**: it is what stops the opening sentence and this clause from resolving to "somewhere else, but still local", which was the round-3 gap. | Nothing plausible. Removing every override and optional store would make it vacuous rather than false, and it would still be safe to leave in a threat model. |
...
**What this line deliberately does not say.** Everything in this paragraph is about `docs/security.md:14` only; `PRIVACY.md` is §4.2's. It does not say "everything", "only", "all", "in two locations", or "instead". It does not assert locality unconditionally: "by default" is in the opening sentence and "need not keep it on this machine" is in the closing one. It does not name the API message database or `--outbox`, because cataloguing stores here would re-open the exhaustiveness trap this rewrite exists to escape and the catalogue is **#1195**'s. It does not claim every inter-agent message is a Markdown file. It does not characterize the Root Agent's own queue at all, which is the legitimate omission the coordinator offered, and it is the safer choice after getting that path wrong once.
...
**Consistency check this closes.** `PRIVACY.md` (§4.2, rewritten alongside this line) and this line now describe one storage model at the same level of confidence: the file-based path writes Markdown into `messaging/` directories under both roots; other delivery paths exist and are not enumerated; locality is stated as the **default** in both, with each document saying outright that a caller-selected destination need not be on this machine; and neither document claims its list is complete. Neither can be falsified by the other, and neither is falsified by the stores and overrides catalogued for #1195.

**This paragraph is itself a consistency claim, so it has to be re-checked whenever either text changes.** It asserted "other delivery paths exist **and stay local**" until the locality correction, which is false by the same `--outbox` falsifier and directly contradicted §4.2's "including on another machine". A stale consistency check is worse than none, because it certifies agreement that is no longer there. Any future edit to §4.2 or §4.6 must end here.
...
**Security and privacy documentation.** `PRIVACY.md` retains the true part of its guarantee, that AC sends message content to no service of its own, and qualifies the part that was not: locality is now stated as the **default** rather than as unconditional, because a documented `send --outbox` destination can be a network path (§4.2). What changed is everything around it. The baseline `docs/security.md:14` told a security-conscious reader that team configuration and inter-agent messages live under `~/.agentscommander/`: team configuration does not live there at all, and messages live there only in part. Round 1 over-swung and asserted the mirror image. Round 2 named both roots and was still categorical, which validation falsified with a store neither round had audited. **Round 3 stops making the claim categorical at all** (§4.6, §4.2): both documents now describe the default AC-managed locations, name the file-based path as one path rather than as all messaging, and say outright that their lists are not exhaustive. Batch 5 (§8.7) ships both.

The honest summary of what this change delivers on the documentation side is therefore narrower than round 2 claimed, and narrower again than the sentence that stood here until the final consistency pass. Stated precisely: **after batch 5 the two replacement blocks, meaning the changed messaging-location and messaging-storage claims, are true, mutually consistent, and explicitly incomplete.** They point a reader at the right directories to start from and tell them the list has a boundary. **This is a claim about those four lines, not about the two documents.** Both files retain untouched sentences outside this change's scope that are themselves inaccurate, including `PRIVACY.md:5` and `:43` hard-coding `~/.agentscommander/` where a portable instance resolves next to the binary (`config/mod.rs:108-112`), and `PRIVACY.md:39` on third parties. Those are catalogued for issue **#1195**, which also owns the complete storage inventory that three rounds established does not yet exist.
...
**If either target does not hold the text you expect.** For `docs/security.md:14` four superseded versions are on record in §4.6: the baseline, the round-1 text that batch 4 shipped, and the round-2 and round-3 texts that were never shipped. For `PRIVACY.md:29-31` three are on record in §4.2: the baseline, the shipped round-1/2 text, and the never-shipped round-3 text. Only the block labelled **Replacement** ships, and it is the same whichever superseded version you find. **Any state not on those lists means someone edited outside this plan: stop and report it.** Do not merge, do not adapt the wording, do not treat a partial match as done.
...
# PRIVACY.md: the guarantee survives in its true, qualified form and every
# universal is gone. Each must return a hit, and all must be on line 31:
git grep -n -F 'local by default'                     -- PRIVACY.md
git grep -n -F 'no service of its own'                -- PRIVACY.md
git grep -n -F '`messaging/`'                         -- PRIVACY.md
git grep -n -F 'file-based path'                      -- PRIVACY.md
git grep -n -F 'other delivery paths'                 -- PRIVACY.md
git grep -n -F 'including on another machine'         -- PRIVACY.md
git grep -n -F 'send --outbox'                        -- PRIVACY.md

# Every superseded phrasing is gone, including the two unconditional locality
# clauses the round-3 text preserved. Each must return exit 1:
...
git grep -n -F 'on this machine by default'                   -- docs/security.md
git grep -n -F 'need not keep it on this machine'             -- docs/security.md
...
git grep -n -F 'its state in files on this machine'           -- docs/security.md
...
| 20 | `PRIVACY.md` contains the literals `local by default`, `no service of its own`, `` `messaging/` ``, `file-based path`, `other delivery paths`, `including on another machine` and `send --outbox`, **all on line 31**; contains **zero** `phone` or `conversations` hits; and all three superseded phrasings (`Messages are written as Markdown files`, `entirely local`, `No external network calls are made`) return **zero** hits | batch 4 greps |
| 21 | `docs/agents/inter-agent-messaging.md` and `docs/security.md` contain **zero** `conversations` hits; `docs/security.md` positively contains all ten D4 anchors (`` `messaging/` ``, `outbox/`, `_team_`, `ac-root-agent/`, `instances/<instance-id>/outbox/`, `<replica>/<instance-dir>/outbox/`, `neither list is exhaustive`, `where to start looking`, `on this machine by default`, `need not keep it on this machine`), **each returning exactly one hit and each on line 14**; and all five superseded phrasings (`project workspace instead`, `Everything is plain files`, `Inter-agent messages live in both locations`, `` `outbox/` delivery queue ``, `its state in files on this machine`) return **zero** hits | batch 4 greps |
...
| 27 | **One storage model across both documents, and neither claims unconditional locality.** `PRIVACY.md` and `docs/security.md` each contain the literal `Root Agent directory`, so neither describes message storage as exclusive to the other's location. Each also carries an explicit default-qualifier on locality (`local by default` and `on this machine by default` respectively) and an explicit statement that a caller-selected destination can be remote (`including on another machine` and `need not keep it on this machine`), all four asserted by criteria 20 and 21. **The claim is deliberately narrower than round 3's "neither document states a universal"**, which was itself an overstatement while `PRIVACY.md` retained the word "entirely"; the eight negative probes of criteria 20 and 21 are what actually enforce it | batch 4 greps |
...
- **§4.6 has four superseded versions on record and §4.2 has three.** Only the block labelled "Replacement" ships. If you are looking at a version that says "instead", "Everything is plain files", "Messages are written as Markdown files", "entirely local", or "its state in files on this machine", you are reading a superseded one.
- **The clauses that matter most are the weak ones.** "you will find", "neither list is exhaustive", "The file-based path", "other delivery paths", **"by default"** in both files. They look like hedging and they are the entire point: every rejected version failed because the text was categorical. Do not tighten them, do not make them read more confidently, and do not "improve" the grammar in a way that restores a universal. **In particular, `PRIVACY.md` no longer says "entirely local" or "No external network calls are made"; both are now negative gate probes.** If you find yourself reinstating a familiar-sounding privacy sentence, stop.
...
| 2 | Exact `PRIVACY.md` replacement wording | Specified verbatim, 3 lines for 3 lines, with a per-clause evidence table. **Rewritten at round 3**, when the shipped universal "every message is a Markdown file in `messaging/`" was falsified by the control-plane API path, **and again at the locality correction**. **No clause is preserved verbatim.** "entirely local" and "No external network calls are made" were both retired: each is unconditional, and `send --outbox` to a network path falsifies both. What survives is the narrower true claim, "AC sends message content to no service of its own", plus "local by default". Both retired clauses are now negative gate probes. | §4.2, §13.6 |
...
| 4a | Which location the rewritten `docs/security.md:14` names for messages | **Both, and it says its list is not closed.** Round 1 said the project workspace "instead", false for Root Agent `messaging/`. Round 2 said both but categorically, and validation falsified the categorical with a fifth store. Round 3 keeps both roots, drops the **false exhaustive-storage quantification**, and disclaims exhaustiveness in the line itself; the locality correction then dropped the unconditional locality claim. **Bounded true quantifiers stay by design**: the line still says "each workgroup's" and "each agent replica's", which are correct and carry the useful information. Ships as batch 5. | §4.6, §4.6.1, §8.7 |
| 4b | Whether `PRIVACY.md`'s §4.2 wording changes | **Not at round 2; yes at round 3, and again at the locality correction.** Rounds 1 and 2 both judged the shipped text correct. It is not: it universally claims every message becomes a Markdown file in `messaging/`, and the control-plane API path creates no file. Round 3 fixed that but preserved "entirely local" and "No external network calls are made" verbatim on instruction; both are themselves unconditional and `send --outbox` to a network path falsifies them. **Every false universal is now gone from the block**: the exhaustive Markdown-storage claim and both unconditional locality clauses. **Bounded true quantifiers stay by design**: "AC sends message content to **no** service of its own" is a universal negative, and it is precisely the claim worth making. Criterion 27 was narrowed for the same overstatement. | §4.2, §9.5 criterion 27 |
| 4f | How locality is stated in both documents | **As a default, with the caller-selected exception stated outright.** `docs/security.md`: "on this machine by default" plus "need not keep it on this machine". `PRIVACY.md`: "local by default" plus "including on another machine", with the surviving true claim narrowed to "AC sends message content to no service of its own". The falsification test is the same one used throughout: would the clause hold if a user pointed `--outbox` at a network share? | §4.6, §4.2 |
| 4g | Where a claim about the two documents may be restated inside this plan | **Nowhere, without re-checking it against the certified text.** Two consecutive corrections were applied to a replacement text while leaving the plan's own prose asserting the pre-correction position: round 2 left the false premise in §4.2, §7 and criteria 20/21/27; the locality correction left "other delivery paths exist and stay local" in §4.6's consistency check, "the privacy guarantee is preserved verbatim" in decision 2, and a blanket "No" on naming `--outbox` in decision 4d. **A restatement is a claim and inherits the same falsification test as the clause it restates.** §4.6's consistency-check paragraph now says so at the point where it is easiest to forget. | §4.6, §11 decisions 2 and 4d, §13.7 |
...
| 4d | Whether to name the API message database and `send --outbox` in these two documents | **Split, and the split is deliberate.** The **API message database is named in neither**: naming it re-enters the exhaustiveness trap, since the next unaudited store falsifies the sentence again. `send --outbox` **is named in `PRIVACY.md`**, as the example in "A destination you select yourself, such as `send --outbox`". It is not an enumeration of stores but an illustration of the caller-selected exception that makes the locality qualifier concrete, and "such as" keeps it exemplifying rather than closed. `docs/security.md` names neither. Round 3 recorded a blanket "No" here; the locality correction changed that for `--outbox`, and the coordinator confirmed keeping it as durable rather than incidental, so criterion 20 now anchors the literal. The full storage audit remains issue **#1195**. | §4.2, §4.6.1, §9.5 criterion 20, §12.2 item 12 |
| 4e | What the two **replacement blocks** claim after batch 5 | **True, mutually consistent, and explicitly incomplete.** This is a deliberate reduction in what those four lines assert, not an oversight; §7 states it plainly rather than implying completeness. **Scoped to the blocks, not to the documents**: both files retain untouched inaccurate sentences (`PRIVACY.md:5`, `:39`, `:43`) that are #1195's, and calling the documents true while deferring their known-false clauses would be the same overstatement in a different place. | §7, §4.6, §13.7 |
...
| 14 | Not addressed. Added by the Step 7 round-3 locality correction | **Both documents also claimed locality unconditionally, and that is false under a documented invocation.** `docs/security.md` said "AC keeps its state in files on this machine"; `PRIVACY.md` said "entirely local" and "No external network calls are made". `send --outbox` (`docs/reference/cli.md:128`) accepts any path with no validation (`cli/send.rs:1078-1079`) and writes the message JSON there (`:1093-1098`), so a UNC or mounted network destination puts the data on another host. Both documents now state locality as the default and name the caller-selected exception. | A privacy policy and a threat model asserting data never leaves the machine, in a product that ships a documented option to send it elsewhere. **Note this survived four earlier passes**, including two where the exhaustiveness problem was being actively fixed: `--outbox` was cited each time as a *location* escape and never as a *locality* escape. |
...
#### What I could not close at round 3, stated rather than certified around

*Read as the round-3 position. Item 2 was overturned by the locality correction (§13.6) and item 3 was narrowed by the final consistency pass (§13.7); both are left here in their original form with the outcome appended, because the record of a wrong call is worth more than a tidy one.*

1. **The corrected text is specified, not implemented.** Batch 5 is the remaining work: two files, four lines. **Still open, and still the only operative item.**
2. **"Entirely local" and "No external network calls are made" are preserved on instruction.** They are true of AC's own behaviour. A user can weaken them without AC changing, by pointing `--outbox` at a network share or widening `apiServerBind` off loopback. Neither makes the sentence false as written; both belong in #1195. Recorded in §4.2 as dissent, not as a blocker. **Overturned.** Filing this as dissent was the wrong call, the reasoning behind "neither makes the sentence false as written" does not hold, and both clauses were retired. §13.6 has the full account.
3. **Neither document is a complete inventory, and both now say so.** That is the decided outcome, not a gap I am hiding: §7 states it, §4.6 states it in the shipped line, and #1195 owns the audit. **Narrowed.** The accurate form is per-block, not per-document: `docs/security.md:14` says so in its own words ("neither list is exhaustive"), and `PRIVACY.md:31` carries it structurally through "other delivery paths" and "such as" rather than as an explicit disclaimer. Both documents also retain untouched inaccurate sentences that are #1195's, so no claim about either document as a whole is being made here.

#### Certification (round 3, superseded by the locality correction)

Certified at Step 7 round 3, digest `1C5FD18BB0BF9BFCE74F95B6BB1150E1324B89BD8922BBA82F2821BD18AEB1C2`. **Withdrawn for the locality correction in §13.6.**

---

### 13.6 Step 7 round-3 locality correction: one clause per document

Not a fourth consensus round. `dev-rust-grinch` rejected the round-3 wording on a single blocking falsifier, the coordinator verified it, identified the contradictory instruction that caused it, retracted that instruction, and prescribed the fix. This section records the correction.

#### The falsifier

Both texts still asserted locality **unconditionally**: `docs/security.md` opened "AC keeps its state in files on this machine", and `PRIVACY.md` kept "entirely local" and "No external network calls are made".

Re-derived rather than accepted: `cli/send.rs:1078-1079` takes `--outbox` as `PathBuf::from(outbox_path)` with **no validation of any kind**; `:1093-1098` calls `create_dir_all` on it and writes `<msg_id>.json` into it. I confirmed `args.outbox` is read at only two sites in the file, `:534` and `:1078`, and that neither applies a locality or `path_identity` check, unlike the PTY-artifact paths at `:310`, `:368` and `:405`. `docs/reference/cli.md:128` documents the option publicly. So `send --outbox \\host\share\...` is a supported invocation under which the message JSON physically lives on another machine, and both claims are false.

Two arguments the coordinator settled, recorded so they are not reopened: **the existing "overrides can put it outside either root" caveat does not cure it**, because that is compatible with the data still being local, and **"the user chose it" does not exempt it**, because Telegram and voice are equally user-initiated and the policy conditions their network claims.

#### My own error, stated plainly

I found this at round 3 and filed it as **recorded dissent instead of as a defect**. My reasoning was that neither clause was false *as written*, because neither is AC calling an external service. That reasoning does not survive contact with what the document is for: `PRIVACY.md`'s subject is where a user's data goes, not which function AC invokes. Under `--outbox` to a network path the data leaves the machine, so "entirely local" is false, and no distinction about who initiated it repairs that. **A clause I had already identified as weakenable should have been raised as blocking rather than as a footnote.** The coordinator's instruction to preserve those clauses verbatim was contradictory with the same message's instruction to remove every categorical, and it is retracted, but the dissent was mine to escalate and I did not.

#### The correction, one clause per document

| Document | Was | Now |
| --- | --- | --- |
| `docs/security.md:14` | "AC keeps its state in files on this machine." and "can put it outside either" | "AC keeps its state in files, **on this machine by default**." and "can put it outside either root **and need not keep it on this machine**" |
| `PRIVACY.md:31` | "**entirely local**" and "No external network calls are made." | "**local by default**", "AC sends message content to **no service of its own**", and "A destination you select yourself, such as `send --outbox`, can place a message outside those locations, **including on another machine**." |

The surviving true claim is the one the coordinator identified: AC transmits message content to no endpoint it operates. That is a statement about **AC's outbound behaviour**, which is what a privacy policy can actually promise, and it is not falsified by a user-selected destination or by an operator widening `apiServerBind` off loopback.

#### Gates

Four positive anchors added (`on this machine by default`, `need not keep it on this machine`, `local by default`, `including on another machine`, plus `no service of its own` replacing the retired guarantee anchors) and four negative probes added (`its state in files on this machine`, `entirely local`, `No external network calls are made`, alongside the existing set). Per §9.3 rule 9(d) the negative set accumulates: **the two retired guarantee clauses are now things the gate forbids**, so a well-meaning restoration of the familiar privacy sentence fails mechanically.

**Criterion 27's overstatement is fixed.** It claimed "neither document states a universal" while `PRIVACY.md` literally retained "entirely". It now asserts the narrower thing that is actually true and gated: each document qualifies locality as a default and names the caller-selected exception.

#### What this correction did not touch, so the fence can be confirmed

The storage enumerations in both documents; the Root Agent handling; the nested replica queue path `<replica>/<instance-dir>/outbox/`; §9.3 rule 9's four properties; the eight pre-existing `docs/security.md` anchors and the gate scoring matrix; batch 5's shape, owner, commit message and two-file scope; every §5 cut coordinate; §1.3's line counts; the 13-path whitelist; §3, §6, §8.0-§8.6. **No code decision was reopened.** `docs/security.md` stays at 121 lines and `PRIVACY.md` at 54: both replacements are the same shape as before, one line for one line and three lines for three lines.

#### Certification (locality correction, superseded by the consistency sweep)

Certified after the locality correction, digest `08ED488A6B9C820EF5AD452BB4F6DC0D0C6567674B2FEBE23BDB2663ED7D6975`. **Withdrawn for the plan-internal consistency sweep in §13.7.** No wording changed; the two replacement texts were validated on truth and are byte-identical across that withdrawal.

---

### 13.7 Plan-internal consistency sweep: the correction did not propagate through the plan's own prose

**The two replacement texts were approved on truth.** Validation constructed falsifiers against every clause, including the two I was least sure of, and none survived: `local by default` and `on this machine by default` hold because a UNC `--outbox` is a caller-selected non-default destination the text now expressly admits, and `AC sends message content to no service of its own` holds because no outbound inter-agent path reaches a service AC operates. The discrimination matrix was independently reproduced exactly, and both replacements were simulated against the complete target files with every positive resolving only to its intended line and the counts landing at 121 and 54.

**What failed was the plan's own prose about those texts.** Three statements still asserted the pre-correction position, two of them contradicting the text certified in the same file. A batch-5 owner reading §4.2 and §11 would have received mutually contradictory canonical instructions, which is precisely the cold-start failure the plan contract exists to prevent.

#### The three, and the three more the sweep found

| Where | Said | Corrected to |
| --- | --- | --- |
| §4.6 consistency check | "other delivery paths exist **and stay local**" | Locality stated as the default in both documents, each naming the caller-selected exception. The paragraph now also states that it is itself a consistency claim and must be re-checked whenever either text changes. |
| §11 decision 2 | "The privacy guarantee **is preserved verbatim**" | **No clause is preserved verbatim.** Both guarantee clauses were retired as unconditional; what survives is the narrower true claim plus `local by default`. Both retired clauses are now negative gate probes. |
| §11 decision 4d | Naming `--outbox`: **"No"** | **Split.** The API database is named in neither document; `send --outbox` is named in `PRIVACY.md` as an example of the caller-selected exception, and criterion 20 now anchors that literal so a future rewrite cannot silently drop it. `docs/security.md` names neither. |
| §4.2 evidence, "other delivery paths" row | "Naming them is #1195's job", stated without qualification | The **clause** names neither; the paragraph's third sentence does name `--outbox` as an example. Both are recorded, with why they are compatible. |
| §4.6.1 standard | "the replacement ... names overrides and optional features as a category **without naming** the API database or `--outbox`" | Scoped to the `docs/security.md` replacement, with the `PRIVACY.md` exception stated in the same place. |
| §4.6 "What this line deliberately does not say" | Read as a claim about both documents, and omitted locality | Scoped explicitly to `docs/security.md:14` in its first sentence, and now lists the locality qualifiers among what the line does not assert. |

#### The pattern, which is now a decision rather than a lesson

**This is the second consecutive correction where a text was fixed and the reasoning around it was not.** Round 2 left the false premise in §4.2, §7 and criteria 20/21/27 after §4.6 was rewritten. The locality correction left three restatements standing after both texts were rewritten. The mechanism is the same both times: a restatement reads as commentary, so it does not get re-checked, but **a restatement is a claim and inherits the falsification test of the clause it restates**. §11 decision 4g records this, and §4.6's consistency-check paragraph carries the obligation at the point where it is easiest to forget.

The sweep was run as three targeted passes over the whole file rather than by reading: locality assertions, verbatim-preservation claims, and claims about what the documents name. It found the three reported plus three more. A second pass over the same three classes after the fixes returns only legitimate hits: quoted superseded text, gate anchor lines, evidence rows describing the change correctly, and the one genuine `Preserved verbatim` (the portable-instance parenthetical, which is carried over from the baseline unchanged).

#### Gate change

One anchor added, per the coordinator's item 4: `send --outbox` as a positive on `PRIVACY.md`, in §9.4 and in criterion 20. Keeping the example is a durable decision rather than incidental wording, so it is now an acceptance criterion. **No other anchor or probe changed**, and no replacement text changed.

#### What this sweep did not touch

Both replacement texts, byte for byte. The storage enumerations, Root Agent handling, the nested replica queue path, rule 9's four properties, every other anchor and negative probe, batch 5's shape and file count, the line counts, the 13-path whitelist, every §5 cut coordinate, §3, §6, §8. **No production file, no code decision.** The two pre-existing `PRIVACY.md` claims validation flagged outside our four-line scope (`:5` and `:43` hard-coding `~/.agentscommander/`, and `:39` on third parties) are the coordinator's to route to **#1195** and are deliberately not widened into here.

#### Final consistency pass: evaluative claims about untouched text

A seventh restatement class, which the three-class sweep above did not cover: **claims about the truth of sentences this change does not touch, and quantifications over whole documents.** Seven statements, all corrected in one pass.

| Where | Was | Now |
| --- | --- | --- |
| §4.2 evidence, "No `~/.agentscommander/` path" row | `PRIVACY.md:5` called a "**still-true**" claim | It is not true: it hard-codes `~/.agentscommander/` while `config/mod.rs:108-112` resolves the config directory next to the executable for a portable instance, `$HOME` being the `:137-142` fallback. The row now says to leave the line alone **because it is out of scope, not because it is correct**, and points at #1195. |
| §7 | "after batch 5 **the two documents** are true, mutually consistent, and explicitly incomplete" | Scoped to **the two replacement blocks**, with the untouched inaccurate sentences named. |
| §11 decision 4e | Same whole-document claim | Same narrowing, and the row title now says "replacement blocks". |
| §11 decision 4a | "drops **every universal quantifier**" | "drops the **false exhaustive-storage quantification**", noting that "each workgroup's" and "each agent replica's" stay by design. |
| §11 decision 4b | "**Every universal** is now gone from the block" | "Every **false** universal", noting that "no service of its own" is a bounded true universal negative and is the claim worth making. |
| §13.5 open item 2 | Stated in the present tense that the two guarantee clauses are preserved and true as written | Marked **overturned**, pointing at §13.6. |
| §13.5 open item 3 | "Neither document is a complete inventory, and **both now say so**" | Marked **narrowed** to per-block, since `PRIVACY.md` carries non-exhaustiveness structurally rather than as an explicit disclaimer. |

The last two were found by sweeping this class and are corrected under the coordinator's standing instruction to fix same-kind findings in the same pass rather than reporting and waiting. §13.5's items are left in their original wording with the outcome appended: **a record of a wrong call is worth more than a tidy one**, and both were mine.

**The common shape of all seven.** Each asserts something about text the change does not modify, or quantifies over a document rather than over the lines being changed. That is a different failure from §13.7's first six, which restated the plan's own decisions. It is the same root, though: **an evaluative sentence feels like summary rather than claim, so it escapes the falsification test.** The operative content was never affected in either case, and no batch-5 owner would have typed a different character; what was at risk was the accuracy of the record.

#### Certification (final)

This plan is certified `READY_FOR_IMPLEMENTATION`. The certified artifact is the committed file; any byte change after the certifying commit invalidates this status and requires re-certification.
