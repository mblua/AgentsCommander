...
A token-authorized two-phase operation on the caller's own session that carries context across a reset. You write `SELF-HANDOFF.md` in your own root first, or the daemon rejects the request; phase 1 waits for 30 seconds of sustained idle and acts, then phase 2 waits a fresh 30 seconds of idle, archives the handoff into `self-clear/`, and injects a resume prompt naming that archive. Three verbs differ in what phase 1 does: `self-handoff-and-clear` injects the provider's logical-clear text in place, `self-handoff-and-switch` respawns the session on a requested coding agent or profile, and `self-handoff-and-restart` respawns it on the same coding agent and profile letter it is already running. Both phases are best-effort in all three, and each verb states its own caller scope. See [`self-handoff-and-clear`](reference/cli.md#self-handoff-and-clear).
