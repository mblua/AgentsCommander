...
The pass is keyed by **command**, not by agent id. Several coding agent profiles can share one binary (a "max effort" `claude` and a "cheap" `claude`, for example), and the binary is what gets updated, so AC asks once for `claude` rather than once per profile. If several catalog entries share one command, the first effective entry for that command supplies its label and exact sequence; later duplicates are ignored.
...
Commands come only from the persisted catalog: AC never substitutes the shipped `updateCommands` at read time, and the catalog's `autoUpdate` field is inert. Only `agentAutoUpdateByCommand` authorizes an update, keyed by the exact command string. If the persisted catalog has no entry for a command, or its first effective sequence is `[]`, that command has no row and never updates.
