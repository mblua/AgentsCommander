...
The matrix is a grid. Each **row** is a coding agent (`claude`, `codex`, `antigravity`, `pi`, or a custom one). Each **column** is a profile letter. Each **cell** is one launch variant for that coding agent and letter:

| | A | B | C |
|---|---|---|---|
| **claude** | (params, env) | (params, env) | (params, env) |
| **codex** | (params, env) | (params, env) | (params, env) |
| **antigravity** | (params, env) | (params, env) | (params, env) |
