...
| `POST /api/v1/pty-input` | `pty_input` | Privileged exact PTY actuation into one live, automatically bound container orchestrator's same-room member. Requires the `pty-input` scope. `text` is 1 through 65,536 decoded UTF-8 bytes. `opId` is permanently idempotent for that sender incarnation. |
...
| `POST /api/v1/terminal-snapshot` | `terminal_snapshot` | Reads one live backend terminal viewport as JSON or PNG. Requires the `terminal-snapshot` scope **and** the `terminalSnapshotsEnabled` gate, which is on by default and denies on an explicit `false`. See [Terminal snapshots](terminal-snapshots.md). |
...
- A live container orchestrator may target one verified non-orchestrator member in the same exact project and room.
- Manual API clients, workers, stale sessions, cross-room and cross-project routes, aliases and wildcards have no privileged authority, whatever their scope list says.
