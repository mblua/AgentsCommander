...
- `<config_dir>` is the authoritative config directory. The canonical Linux
  DEB uses `$XDG_CONFIG_HOME/agentscommander`, with
  `$HOME/.config/agentscommander` as its fallback. Raw portable binaries use
  their executable-relative directory. See
  [Portable instances](portable-instances.md#config-directory-rule).
