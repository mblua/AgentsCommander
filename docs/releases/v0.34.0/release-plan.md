# Release preparation plan: v0.34.0
...
Candidate: `0.34.0` / `v0.34.0`  
Planning base: `2c4999508a35929b0ee6dbcfa7f7f2bab0201cb7`  
Predecessor: `v0.33.0`
...
The annotated-tag message is the exact bytes of `release-authority-v1.txt`. Its `review-set-sha256` binds the plan, evidence manifest, changelog input, Release body, and both asset ledgers without a circular self-hash. The candidate tree must contain every bundle file at `docs/releases/v0.34.0/` before tagging.
...
- `docs/releases/v0.34.0/CHANGELOG.release.md`
- `docs/releases/v0.34.0/candidate-assets.v1.json`
- `docs/releases/v0.34.0/input-manifest.v1.json`
- `docs/releases/v0.34.0/predecessor-assets.v1.json`
- `docs/releases/v0.34.0/release-authority-v1.txt`
- `docs/releases/v0.34.0/release-body.md`
- `docs/releases/v0.34.0/release-plan.md`
- `docs/releases/v0.34.0/SHA256SUMS`
...
- Git remote main and GitHub API agree at `2c4999508a35929b0ee6dbcfa7f7f2bab0201cb7`.
- Ordered planning-base parents: [a431c7bb523ade19aa9b0d0e86ab21ec300ff8ae, 0608fe0166bfcaad2052cf82eebec257c596489f].
...
- Predecessor annotated object: `7bbd680ea8ce8e3840c878264351bf06f78a56e1`; peeled commit: `f83189a6dc3de49532670c8a2179590b7f89b19e`.
- Predecessor immutable GitHub Release id: `389289577`.
- npm latest: `0.33.0`; candidate tag, Release, and npm version are absent at both discovery snapshots.
...
- `package.json` `/version`: `0.33.0` (blob `6a90dae4ffc522fe05c909fd0f35f05ae05a45bf`)
- `package-lock.json` `/version`: `0.33.0` (blob `b6e4b505e360b26edd5cdad0f42a36b013567635`)
- `package-lock.json` `/packages//version`: `0.33.0` (blob `b6e4b505e360b26edd5cdad0f42a36b013567635`)
- `npm/package.json` `/version`: `0.33.0` (blob `801cb9612177a908abb835e5a47ccb63a0588119`)
- `npm/install.js` `const VERSION`: `0.33.0` (blob `1819959908b2bc98fcbe7e23129cf86cd0b58cdc`)
- `src-tauri/Cargo.toml` `[package].version`: `0.33.0` (blob `2edae61954f9f1b7608a067021a3a473b275ab65`)
- `Cargo.lock` `agentscommander.version`: `0.33.0` (blob `b8eda6af06a85a6768699e0effe0a06ee4be2a55`)
- `src-tauri/tauri.conf.json` `/version`: `0.33.0` (blob `8a32250afd4548e56cb8167dd2da51ace00403b2`)
...
Before implementation, independently query remote main through raw Git and the GitHub ref API. Both must equal `2c4999508a35929b0ee6dbcfa7f7f2bab0201cb7`; the GitHub commit API must return the ordered parent list [a431c7bb523ade19aa9b0d0e86ab21ec300ff8ae, 0608fe0166bfcaad2052cf82eebec257c596489f]; the contents API must return workflow blob `be57abaaa94ee8ad452a78b12230f61a44851d05`. Any mismatch is `FROZEN_INPUT_CHANGED`: discard the bundle and run the generator again from a new config/output path.
...
docs/releases/v0.34.0/release-plan.md
docs/releases/v0.34.0/SHA256SUMS
plans/2071-v0340-release-hardening.md
```
...
```text
[2c4999508a35929b0ee6dbcfa7f7f2bab0201cb7, HARDENING_HEAD]
```
...
Replace the exact current `Unreleased` body once: remove it from `Unreleased` and insert it under `## 0.34.0`. The expected complete result is `CHANGELOG.release.md`; use `release-body.md` verbatim for the GitHub Release. Duplicate or residual copies fail.
...
Run the repository version tool for `0.34.0`; do not hand-edit version surfaces. Require all eight parsed surfaces to equal the candidate on the version PR head. Run `npm pack --dry-run`, create the tarball, inspect it, and smoke-install from that exact tarball before tagging.
...
After explicit authorization, create one annotated tag at `FINAL_CANDIDATE_MAIN` using the exact `docs/releases/v0.34.0/release-authority-v1.txt` bytes. Before push, require the local tag object type to be `tag`, its payload to match byte-for-byte, and its peeled commit to equal `FINAL_CANDIDATE_MAIN`.
...
Completion requires all 17 candidate assets and checksums, a public immutable GitHub Release, verified attestations, npm dist-tag/version/provenance/signature, clean npm install, and the destination executable reporting `0.34.0`. WG23 independently repeats GitHub/npm/executable verification before declaring the release complete.
