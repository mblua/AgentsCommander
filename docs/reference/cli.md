...
| `--agent` | No | Configured coding-agent id used when `wake` spawns or respawns the target's persistent session (file notifications, logical commands, and PTY input alike). `auto` picks the destination's saved `lastCodingAgent`. When the target is already live the flag cannot apply; the delivery proceeds and the `Delivered:` receipt states the flag was not applied. It never selects a session, executable, or backend directly. |
| `--profile` | No | Profile slot letter `A-Z` applied to the coding agent the `wake` spawns or respawns (the agent given by `--agent`, or the one `auto` resolves). A disabled or missing letter walks down to the nearest enabled cell and the `Delivered:` receipt reports `fallbackApplied`. Never writes the replica's `tooling.profile`, `currentCodingAgent` or `lastCodingAgent`. For this spawn it outranks a pinned replica profile. Cannot be combined with `--pty-input` or `--pty-input-stdin`. When the target is already live the flag cannot apply; the receipt states it. |
