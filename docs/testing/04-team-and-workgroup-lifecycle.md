# 04 Team, Room, And Peer Lifecycle

These cases validate that a user can compose project agents into a team, activate that team into a room from the GUI, and verify room-specific peer discovery and isolation.
...
- Until New Team/New Room selectors exist and onboarding persistence is fixed, this suite should be treated as blocked for automated baseline PASS.

Known automation support:

- Room replica rows have some semantic selectors for quick/room contexts, mainly for repo badge verification.
- `list-peers-lean` provides canonical peer names and reachability for cross-checking GUI-visible room participants.

Known automation gaps:

- Team and Room section headers, context menus, New Team modal fields, agent checkboxes, orchestrator radio buttons, repository assignment controls, New Room modal fields, team select, task-title input, team rows, room rows, and agent rows do not yet have stable semantic selectors.
...
### WGP-000: Team and room creation controls guard incomplete input

Purpose:

Verify that team and room creation dialogs do not create entities until required inputs are present.
...
7. If a team already exists, open New Room.
8. Confirm `Create` is disabled or blocked until a team and non-empty task title are present.
9. Cancel New Room.
10. Confirm no new `room-*` directory was created.

Expected Result:

Incomplete or canceled entity creation flows do not create team or room state.

Evidence Required:

- Screenshots showing disabled/blocked Next or Create states.
- Screenshot of Back/Cancel path when possible.
- Read-only filesystem snapshot proving no new `_team_*` or `room-*` directory was created.
...
### WGP-002: Activate a room from a team

Purpose:

Verify that a user can activate a team into a new room from the GUI.
...
1. Open the project context menu or Rooms section context menu.
2. Choose `New Room`.
3. Select the team created in WGP-001.
4. Enter the unique task title.
5. Click `Create`.
6. Wait for room creation and sidebar refresh.
7. Expand the Rooms section.
8. Inspect the new room and its replicas.

Expected Result:

The app creates a new `room-<N>-<team>` directory, copies team agents as replicas, writes `TASK.md`, refreshes the sidebar, and shows the room with its members.

Evidence Required:

- Screenshot of the filled New Room modal.
- Screenshot of the Rooms section after creation.
- Read-only filesystem snapshot showing the new room directory, `TASK.md`, replica directories, and messaging directory.
- Any progress or error message shown during creation.

Pass/Fail Criteria:

Pass if the room is created from the GUI and appears with the expected team replicas. Fail if creation errors, clones wrong agents, writes no task, or does not refresh.

### WGP-003: Launch orchestrator session from created room

Purpose:

Verify that the newly created room exposes a usable orchestrator replica entry and session launch path.

Preconditions:

- Depends on WGP-002.
- At least one configured coding agent exists.
- The selected orchestrator agent is visible in the room.

Steps:

1. Click the orchestrator replica row in the room.
...
4. Confirm the session identity matches the room replica.

Expected Result:

The orchestrator replica launches or focuses a terminal session for the correct room replica.

Evidence Required:

- Screenshot of the room before launching.
...
Pass if the orchestrator session starts or focuses with the expected room identity. Fail if the wrong replica launches, no session starts, or the session starts outside the room.

### WGP-004: Peer discovery matches visible room participants

Purpose:

Verify that CLI peer discovery aligns with GUI-visible disposable room participants.
...
1. Capture the GUI participant list for the selected disposable room.
2. From the disposable sender root, run `list-peers-lean` with the current token/root.
3. Save peer discovery stdout/stderr.
4. Confirm each tested peer JSON entry uses a canonical `name` and belongs to the expected room/team.
...
Peer discovery returns canonical names for reachable disposable peers that match the visible room participants and routing rules.
...
### WGP-005: Multiple rooms remain isolated

Purpose:

Verify that two disposable rooms do not bleed peer/session state into each other.

Preconditions:

- Depends on WGP-002.
- The tester can create or select a second disposable room without mutating live state.
- Each room has distinct names or participant markers.

Steps:

1. Capture the first disposable room's participants, sessions, and peer JSON.
2. Create or select a second disposable room with a distinct title/name.
3. Capture the second room's participants, sessions, and peer JSON.
4. Switch back to the first room and capture its participant/session list again.
5. Compare visible room names, roots, peer names, and sessions.
6. Record any shared team membership that is expected but confirm replica/session identity remains room-specific.

Expected Result:

Each room retains its own visible participants, replicas, sessions, peer names, and room root. State from one room does not appear as active state in the other.
...
PASS if peer/session state remains isolated by room. PARTIAL if isolation is clear but one optional peer JSON artifact is missing. FAIL if sessions, peers, or roots bleed across rooms. BLOCKED if a second disposable room cannot be created safely.

### WGP-006: Room and peer state refreshes after restart

Purpose:

Verify that visible rooms and peer/session state remain predictable after restarting the testable app.

Preconditions:

- Depends on WGP-002 and WGP-004.
- The disposable room state has been captured before restart.
- The tester can close and relaunch only the testable app.

Steps:

1. Capture the pre-restart room list, selected room, participants, sessions, and peer JSON.
...
5. Capture the post-restart room list and selected/active room.
...
After restart, disposable rooms, selected room behavior, participant visibility, and peer discovery state are coherent and do not introduce live-state bleed or duplicate entries.
...
PASS if post-restart state matches documented recovery behavior and remains isolated. PARTIAL if GUI state is coherent but peer discovery cannot be rerun after restart. FAIL if rooms duplicate, disappear unexpectedly, or mix state. BLOCKED if restart cannot be performed without affecting live windows.
