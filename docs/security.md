...
3. **The optional network endpoints.** Telegram Bot API, the Google Gemini API for voice-to-text, and the Jev classification endpoint (Typesafe System One, `jev_endpoint`, by default `https://api.typesafe.ai/v1/systemone`). Only contacted when the user explicitly enables those features. Jev is the one that exports agent content: it sends the **captured candidate text and the category catalog's questions**, and only when a room's [Co-managed](features/co-managed-rooms.md) flag is on **and** a Jev API key is configured. A room without the flag, or an empty key, makes zero Jev calls.
