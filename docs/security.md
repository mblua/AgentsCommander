...
3. **The optional network endpoints.** Telegram Bot API, the Google Gemini API for voice-to-text, and the Jev classification endpoint (Typesafe System One, `jevEndpoint`, by default `https://api.typesafe.ai/v1/systemone`). Only contacted when the user explicitly enables those features. Jev is the one that exports agent content: it sends the **captured candidate text and the category catalog's questions**, and only when the global `coManagedEnabled` switch is on, a room's [Co-managed](features/co-managed-rooms.md) flag is on **and** a Jev API key is configured. The switch is off by default while the feature is in development; with it off AC makes zero Jev calls, whatever the rooms and the key say. A room without the flag, or an empty key, also makes zero Jev calls.
...
- Send any telemetry. No analytics, no crash reports. The one automatic network check is the npm update check: on startup AC queries the npm registry for the latest published version (throttled to at most once per 24 hours, fail-silent, opt-out via `npmUpdateNotificationsEnabled: false`). No user data leaves the machine in the npm update check; it is a plain version lookup.

**The named exception to that last clause** is Co-managed: when you turn on the global `coManagedEnabled` switch, turn a room's Co-managed flag on and configure a Jev API key, AC sends captured agent text and your catalog's questions to `jevEndpoint`. That is opt-in, globally and per room, and off by default, but it is user content leaving the machine, so it is stated here rather than folded into the update-check sentence. A **pre-egress secret detector** runs first, **before any file is written and before any network call**: a flagged candidate produces no file, no request and no excerpt, and the reason you see names the rule and the candidate's byte length only. See [Co-managed rooms](features/co-managed-rooms.md) and [`PRIVACY.md`](../PRIVACY.md).
...
There is no content redaction. The screen can contain credentials, source, prompts, and personal data. The disclosure setting is `true` by default, for a fresh installation and for an older settings file that has no key, so treat the capability as live unless you have written an explicit `false`. Both strict on-disk state and managed in-memory state are rechecked initially and finally, and an explicit `false` in either denies. Malformed, duplicated, wrongly typed, linked, or unreadable security settings fail closed.
