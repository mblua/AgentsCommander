 ...
 *
 * `target` is the window label this listener wants events for. Tauri's JS
 * default registers a listener as `EventTarget::Any`, and the Rust side
 * SHORT-CIRCUITS the label filter for such a listener
 * (`tauri-2.10.3/src/event/listener.rs:306-311`, `match_any_or_filter`), so an
 * `Any` listener receives every `emit_to` regardless of its label. Passing a
 * string here registers `AnyLabel`, which the backend's `emit_to(label, ...)`
 * filter matches, and which is what makes backend-side scoping actually bind.
 *
